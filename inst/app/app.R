...
# v2 – HITL factor editor, bug fixes, stats tab removed
...
# Single file read: reads lines once, returns list(meta, dat)
read_incucyte_file <- function(path, drop_stderr = TRUE) {
  ...
  dat <- read_delim(I(txt), delim = "\t", show_col_types = FALSE,
...
# ---------------------------
# Canonicalization (vectorized wrappers)
# ---------------------------
...
extract_receptors <- function(x) {
  ...
  if (str_detect(s, "\\bpgr\\b") || (str_detect(s, "\\bpr\\b") &&
                                     !str_detect(s, "\\bpra\\b|\\bpr a\\b|\\bprb\\b|\\bpr b\\b")))
    recs <- c(recs, "PR")
  ...
  canon_parts <- if (canonical == "none") character(0) else
...
extract_treatment <- function(x) {
  s <- clean_condition_header(x)
  veh_pats <- c("\\bveh\\b", "\\bvehicle\\b", "\\be2oh\\b", "\\bethanol\\b",
                "\\betoh\\b", "\\bdmso\\b")
  ...
  norm_unit <- function(z) { z <- toupper(z); if (z == "\u00b5M") "uM" else z }
  ...
  while (i <= length(tokens)) {
    if (used[i]) { i <- i + 1; next }
    # ligand num unit
    if (i + 2 <= length(tokens) && !any(used[i:(i+2)]) &&
        is_ligand(tokens[i]) && is_num(tokens[i+1]) && is_unit(tokens[i+2])) {
      hits <- c(hits, paste0(tokens[i+1], norm_unit(tokens[i+2]), " ", norm_lig(tokens[i])))
      used[i:(i+2)] <- TRUE; i <- i + 3; next
    }
    # num unit ligand
    if (i + 2 <= length(tokens) && !any(used[i:(i+2)]) &&
        is_num(tokens[i]) && is_unit(tokens[i+1]) && is_ligand(tokens[i+2])) {
...
# Extract passage label from condition string (e.g. "p14" -> "Passage_14")
...
# ---------------------------
# HITL: parse all unique conditions from all loaded files
# Returns one row per unique (file, condition) with guesses
# ---------------------------
parse_conditions_hitl <- function(raw_tbl) {
  raw_tbl %>%
    distinct(condition_id, file, well_id, replicate_id, condition) %>%
    mutate(
      ...
      # Editable columns (start as guesses)
      receptor  = receptor_guess,
      treatment = treatment_guess,
      passage   = passage_guess,
      exclude   = FALSE
    ) %>%
    select(exclude, file, well_id, condition, receptor, treatment, passage,
           receptor_guess, treatment_guess, passage_guess, condition_id, replicate_id)
}
...
read_plate_map <- function(path) {
  ...
  pm %>%
    mutate(
      ...
      receptor_pm  = canonicalize_receptor_one(receptor),
      treatment_pm = canonicalize_treatment_one(hormone),
      passage_pm   = if_else(
...
make_plate_preview_tables <- function(df) {
  df <- df %>%
    mutate(
      ...
      label    = if_else(
        well_id == "" | is.na(well_id), NA_character_,
        paste0(as.character(receptor), "\n",
...
ols_control_adjust <- function(df, sig_col, ctl_col) {
  ...
  if (sum(ok) < 3) { df$value_norm <- NA_real_; return(df) }
  fit  <- lm(y[ok] ~ x[ok])
  b    <- unname(coef(fit)[2])
...
treatment_color_values <- c(
  "VEH"="#000000","E2"="#FB0280","P4"="#FD8008","DHT"="#0F80FF",
  "NET"="#00C896","4-OHT"="#7A3CFF","RU-486"="#9E9E9E","Glucocorticoid"="#00A878",
  "E2 + P4"="#C23B8E","E2 + DHT"="#8A4DFF","E2 + NET"="#5BB8A0",
  "E2 + 4-OHT"="#B04DFF","E2 + RU-486"="#B85A8A",
  "P4 + DHT"="#7F9CFF","P4 + 4-OHT"="#C06A88","P4 + RU-486"="#C08A60",
  "DHT + 4-OHT"="#4F5BFF","DHT + RU-486"="#6B8BB8","4-OHT + RU-486"="#8E6BA8"
)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      fileInput("files", "Upload Incucyte export files (.txt/.tsv/.csv)",
    ...
    mainPanel(
      tabsetPanel(
        # Tab 1: Plate map / check
        tabPanel(
          "Plate map",
          br(),
          fileInput("platemap", "Upload plate map (.csv)",
        ...
        # Tab 2: Factor editor (HITL)
        tabPanel(
          "Factor editor",
          br(),
          tags$p(
            tags$strong("How it works:"),
            " The app guesses receptor, treatment, and passage from each well's label.",
            " Edit any cell — changes to a receptor/treatment/passage field",
            " automatically propagate to all wells with the same original guessed value",
            " (shown in the 'guessed' columns). Use 'Apply edits' to lock them in before plotting."
          ),
          br(),
          fluidRow(
            ...
            column(3, actionButton("reset_editor",    "Reset all edits")),
            column(3, actionButton("apply_editor",    "Apply edits", class = "btn-primary"))
          ),
          br(),
          # Quick-fill bulk override panel
          wellPanel(
            h5("Bulk override (fill selected rows)"),
            fluidRow(
              column(3, textInput("bulk_receptor",  "Receptor", placeholder = "e.g. ER_a")),
              column(3, textInput("bulk_treatment", "Treatment", placeholder = "e.g. 1nM E2")),
              column(3, textInput("bulk_passage",   "Passage", placeholder = "e.g. Passage_14")),
              column(3, br(), actionButton("bulk_apply", "Fill selected", class = "btn-info"))
            )
          ),
          rHandsontableOutput("editor_table", height = "520px")
        ),
        ...
        tabPanel(
          ...
          fluidRow(
            ...
            column(4, br(),
                   downloadButton("download_auc_plot_png", "Export AUC plot PNG"), br(), br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$files, {
    req(input$files)
    new_files <- tibble(file = input$files$name, path = input$files$datapath)
    old_files <- uploaded_files_rv()
    combined  <- if (is.null(old_files)) new_files else
      bind_rows(old_files, new_files) %>% distinct(file, .keep_all = TRUE)
    uploaded_files_rv(combined)
  })
  
  observeEvent(input$clear_files, { uploaded_files_rv(NULL) })
  ...
  output$channel_map_ui <- renderUI({
    ...
    tagList(
      ...
      tags$p(tags$small("Files are retained until cleared or app reloads.")),
      lapply(seq_along(fns), function(i) {
        fname <- fns[i]
        default <- if (str_detect(tolower(fname), "red|nir")) "NIR" else
          if (str_detect(tolower(fname), "gfp|green")) "GFP" else
            if (str_detect(tolower(fname), "orange")) "Orange" else "Other"
        fluidRow(
          column(8, tags$small(fname)),
          column(4, selectInput(paste0("chan_", i), NULL,
                                c("GFP","NIR","Orange","Red","Other"), selected = default))
        )
      })
    )
  })
  ...
  output$norm_ui <- renderUI({
    ...
    tagList(
      ...
      selectInput("control_channel", "Control channel", choices = chans,
                  selected = if ("NIR" %in% chans) "NIR" else chans[min(2, length(chans))]),
      radioButtons("norm_method", "Method",
                   choices = c(
  ...
  # ---- Core import (runs on button press) ----
  # raw_long_auto_base: one row per (file, well, channel, timepoint) — no editor applied yet
  raw_long_auto_base <- eventReactive(input$run, {
    ...
    purrr::pmap_dfr(cm, function(file, path, channel) {
      ...
      default_passage <- if (!is.na(meta$passage[[1]]) && meta$passage[[1]] != "")
        paste0("Passage_", meta$passage[[1]]) else "Passage_NA"
      
      # Pivot long
      cond_names <- names(dat_raw)[-(1:2)]
      long <- dat_raw %>%
        pivot_longer(cols = -c(datetime, elapsed),
  ...
  # ---- HITL editor initialization ----
  # Parse conditions once from raw data; editor_rv holds the user-edited version
  ...
  # Initialize or reset editor from fresh parse
  observeEvent(hitl_default(), { editor_rv(hitl_default()) })
  observeEvent(input$reset_editor, { req(hitl_default()); editor_rv(hitl_default()) })
  
  # ---- Render the handsontable ----
  output$editor_table <- renderRHandsontable({
    req(editor_rv())
    df <- editor_rv() %>%
      select(exclude, file, well_id, condition,
             receptor, treatment, passage,
             receptor_guess, treatment_guess, passage_guess)
    
    rhandsontable(df, rowHeaders = NULL, stretchH = "all",
                  height = 500, selectCallback = TRUE) %>%
      ...
      hot_table(highlightCol = TRUE, highlightRow = TRUE,
                columnSorting = TRUE, manualColumnMove = TRUE)
  })
  
  # ---- Sync edits from table back to editor_rv ----
  # Use bindEvent to avoid race condition on startup
  observeEvent(input$editor_table, {
    ...
    tbl <- as_tibble(tbl) %>%
      mutate(across(c(exclude), as.logical),
             across(c(file, well_id, condition, receptor, treatment, passage,
                      receptor_guess, treatment_guess, passage_guess), as.character))
    
    # Propagate: if a receptor/treatment/passage was edited relative to its guess,
    # apply that same edit to all rows sharing the same guess value
    prev <- editor_rv()
    
    # Build a map from (guess -> edited value) for each field
    propagate_field <- function(df_new, df_old, edited_col, guess_col) {
      # Find rows where the edited value differs from the guess (user changed something)
      changed <- df_new %>%
        filter(.data[[edited_col]] != .data[[guess_col]]) %>%
        select(all_of(c(guess_col, edited_col))) %>%
        distinct()
      if (nrow(changed) == 0) return(df_new)
      # Apply: for each unique guess value that was changed, update all rows with that guess
      for (k in seq_len(nrow(changed))) {
        g <- changed[[guess_col]][k]
        v <- changed[[edited_col]][k]
        df_new[[edited_col]][df_new[[guess_col]] == g] <- v
      }
      df_new
    }
    
    # Re-attach condition_id and replicate_id (not shown in handsontable)
    key_map  <- prev %>% select(condition_id, replicate_id, file, well_id, condition)
    tbl_full <- tbl %>%
      left_join(key_map, by = c("file","well_id","condition"))
    
    tbl_full <- propagate_field(tbl_full, prev, "receptor",  "receptor_guess")
    tbl_full <- propagate_field(tbl_full, prev, "treatment", "treatment_guess")
    tbl_full <- propagate_field(tbl_full, prev, "passage",   "passage_guess")
  ...
  observeEvent(input$exclude_selected, {
    ...
    if (length(rows) > 0) { df$exclude[rows] <- TRUE; editor_rv(df) }
  })
  ...
  observeEvent(input$include_selected, {
    ...
    df   <- editor_rv()
    rows <- rows[rows >= 1 & rows <= nrow(df)]
    if (length(rows) > 0) { df$exclude[rows] <- FALSE; editor_rv(df) }
  })
  
  # ---- Bulk fill selected rows ----
  observeEvent(input$bulk_apply, {
    ...
    r <- str_trim(input$bulk_receptor)
    t <- str_trim(input$bulk_treatment)
    p <- str_trim(input$bulk_passage)
    if (nchar(r) > 0) df$receptor[rows]  <- canonicalize_receptor_one(r)
    if (nchar(t) > 0) df$treatment[rows] <- canonicalize_treatment_one(t)
    if (nchar(p) > 0) df$passage[rows]   <- p
    editor_rv(df)
  })
  
  # ---- Committed editor map (after Apply button) ----
  ...
  # Current effective map: applied if available, else live editor state
  current_editor_map <- reactive({
    if (!is.null(applied_editor_rv())) {
      applied_editor_rv()
    } else if (!is.null(editor_rv())) {
      editor_rv() %>%
        mutate(
          exclude    = if_else(is.na(exclude), FALSE, as.logical(exclude)),
          receptor   = canonicalize_receptor_combo(receptor),
  ...
  output$plate_check_layout <- renderUI({
    ...
    tagList(purrr::map(plate_tables, function(x) {
      tagList(
  ...
  observe({
    ...
    purrr::walk(plate_tables, function(x) {
      local({
        ...
        output[[oid]] <- renderTable({ ptbl }, striped = TRUE, bordered = TRUE, spacing = "xs")
      })
    })
  })
  ...
  raw_long <- reactive({
    ...
    em <- current_editor_map() %>%
      select(condition_id, exclude, passage, receptor, treatment, factor_key)
    
    raw_long_auto_base() %>%
      left_join(em, by = "condition_id") %>%
      mutate(
        exclude    = if_else(is.na(exclude), FALSE, as.logical(exclude)),
      ...
      filter(!exclude) %>%
      relocate(exclude, well_id, replicate_id, passage, receptor, treatment,
               factor_key, .after = condition)
  })
  
  # ---- Wide join for normalization ----
  ...
  normalized_passage <- reactive({
    ...
    if (is.null(sig) || is.null(ctl) || !(sig %in% names(w)) || !(ctl %in% names(w))) {
      ...
      if (method %in% c("none","ratio","log2ratio")) {
        out <- out %>% mutate(value_norm = case_when(
    ...
    if (isTRUE(input$mask_spikes)) {
      out <- out %>%
        ...
        ungroup() %>%
        mutate(value_norm = value_masked) %>% select(-value_masked)
    } else {
  ...
  output$plot_time_ui <- renderUI({
    ...
    sliderInput("plot_time_range", "Elapsed time range",
  ...
  auc_preview <- reactive({
    ...
    filtered_stats_long() %>%
      ...
      mutate(auc_valid = is.finite(auc)) %>%
      { n_na <- sum(!.$auc_valid)
      if (n_na > 0) message(n_na, " passage(s) had <2 finite points; AUC set to NA.")
      . } %>%
      select(-auc_valid)
  })
  ...
  output$plot <- renderPlot({
    ...
    if (all(is.na(df$value_norm)) || nrow(df) == 0) {
      plot.new(); text(0.5, 0.5, "No data remain after current filters."); return()
    }
    
    # Show spike-flagged points as red X
    df_ok    <- df %>% filter(!spike_flag | is.na(spike_flag))
    df_spike <- df %>% filter(spike_flag)
    
    p <- ggplot(df_ok, aes(x = elapsed, y = value_norm,
                           group = factor_key, color = receptor)) +
      geom_line(alpha = 0.6, na.rm = TRUE) +
      facet_wrap(~ passage) +
      labs(x = "Elapsed time", y = "Value (normalized)",
           title = "Normalized trajectories (facet by Passage; color by receptor)") +
      theme_minimal()
    
    if (nrow(df_spike) > 0)
      p <- p + geom_point(data = df_spike, shape = 4, color = "red", size = 2,
  ...
  auc_plot_obj <- reactive({
    ...
    df <- df %>%
      mutate(
        ...
        treatment_group = factor(treatment_group,
                                 levels = unique(c(treatment_levels_master,
                                                   sort(setdiff(unique(treatment_group),
    ...
    ggplot(df, aes(x = receptor, y = auc, color = treatment_group)) +
      ...
      labs(x = "Receptor", y = "AUC", color = "Treatment",
  ...
  output$auc_plot <- renderPlot({
    p <- auc_plot_obj()
    if (is.null(p)) { plot.new(); text(0.5, 0.5, "No AUC values for current filters."); return() }
    p
  })
  ...
  output$download_prism <- downloadHandler(
    ...
    content  = function(file) {
      ...
      col_template <- auc_df %>% count(treatment, name = "n_rep") %>%
        group_by(treatment) %>% summarise(max_rep = max(n_rep), .groups = "drop") %>%
        mutate(col_keys = purrr::map2(treatment, max_rep,
                                      ~ paste0(.x, "__rep", seq_len(.y)))) %>%
        pull(col_keys) %>% unlist()
      ...
      write.table(out, file = file, sep = ",", row.names = FALSE,
  ...
  output$download_auc_plot_png <- downloadHandler(
    ...
    content  = function(file) {
      ...
      if (is.null(p)) {
        ...
        plot.new(); text(0.5, 0.5, "No AUC values."); dev.off(); return()
      }
      df   <- auc_preview() %>% mutate(treatment_group = purrr::map_chr(treatment, classify_treatment_group))
  ...
  output$download_auc_plot_svg <- downloadHandler(
    ...
    content  = function(file) {
      ...
      df   <- auc_preview() %>% mutate(treatment_group = purrr::map_chr(treatment, classify_treatment_group))
      dims <- compute_auc_export_dims(df)
      ggsave(filename = file, plot = p, device = "svg",
             width = dims$width_mm, height = dims$height_mm,
             units = "mm", bg = "white")
    }
  )
